...
— 86 numbered test IDs (`T-SS-NN`), all implemented, plus property-based tests
(hypothesis) and multi-seed Monte-Carlo runs. **354 engine tests + 119 API tests
...
- **Cross-provider fusion**
  ...
  *caught a blind spot in our own pipeline* (StatsBomb records own goals as
  ...
  construction. The pipeline also covers the **Premier League, Serie A and
  Ligue 1 2023/24** (380/380, 380/380 and 306/306 fixtures resolved across
  football-data × openfootball, zero conflicts — structural-token
  normalization handles each country's naming conventions generically), and
  `priors_from_agreement` closes the feedback loop: measured reliabilities
  replace the configured priors deterministically — re-fusing with them
  changes 0 values. Fused records persist to Postgres — 1,372 fixtures across
...
### 5.11 Outcome accuracy: can the Future Sim call the final result?

The direct question — *stop a real match at minute t; how much probability
does the leakage-free Future Sim put on the outcome that actually happened?*
Measured over the **full Copa América 2024** (32 matches, known results),
against two honest baselines
([`results/RESULTS_OUTCOME.md`](./results/RESULTS_OUTCOME.md), regenerated by
`scripts/outcome_accuracy.py`; the run re-simulates a sample of checkpoints
with every future event erased and byte-compares — leakage-audited in-run):

| Minute | Engine P(truth) | Engine accuracy | Brier: engine / persist / constant |
|---|---|---|---|
| 0' | 0.341 | 47% | 0.662 / 1.438 / **0.639** |
| 45' | 0.514 | 62% | **0.487** / 0.938 / 0.639 |
| 85' | 0.745 | 84% | **0.250** / 0.312 / 0.639 |

Honest reading: at kick-off the engine is (correctly) barely better than the
prior — football is genuinely random and nothing honest can "call" a match at
0'. As real information arrives it converges: by 85' it holds ~0.75 on the
true outcome with 84% accuracy, and it beats the naive
"current-score-persists" rule on Brier at **every** checkpoint (hedging is the
edge: a 1-goal lead at 45' is fragile; the naive rule treats it as decided).
Calibrated, not clairvoyant — by design.
...
pytest -q                                     # 354 tests, incl. slow MC
